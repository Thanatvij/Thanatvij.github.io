...
// Every page should get the same motion effects (Lenis, scroll reveal, hero spotlight). "/" and "/projects/" are Astro (GSAP + Lenis); the rest are static
...
for (const path of pages) {
  test(`fine pointer: Lenis, reveal and hero spotlight on ${path}`, async ({ page }) => {
    await page.goto(path);
    await page.mouse.move(300, 300); await page.mouse.move(340, 320);
    await expect(page.locator('.cursor, .cursor-ring')).toHaveCount(0);   // the cursor ring was removed on purpose
  ...
  test(`reduced motion: no Lenis, content visible on ${path}`, async ({ browser }) => {
  ...
  test(`touch: no Lenis on ${path}`, async ({ browser }) => {
